...
```sh
docker run --rm -v ${PWD}:/docs sig9/zensical:0.0.16 zensical build --clean
```
...
build-job:
    stage: build
    script:
        - docker run --user $(id -u):$(id -g) --rm -v ${PWD}:/docs sig9/zensical:0.0.16 zensical build --clean
        - rm -rf /var/www/html/*
